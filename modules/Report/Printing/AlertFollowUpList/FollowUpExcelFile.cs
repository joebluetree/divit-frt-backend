...
{
    public class FollowUpExcelFile
    {
        ...
        private void CreateExcelData()
        {
            ...
            foreach (rep_followup_dto dr in Dt_List)
            {
                ...
                excel.CellValue(rowIndex, colIndex, dr.cf_mbl_refno!, excel.GetCellFormat(Border:BL));
                excel.CellValue(rowIndex, colIndex + 1, dr.cf_assigned_name!, excel.GetCellFormat(Border:BL));
                excel.CellValue(rowIndex, colIndex + 2, cf_followup_date.ToUpper()!, excel.GetCellFormat(Border:BL));
                excel.CellValue(rowIndex++, colIndex + 3, dr.cf_remarks!, excel.GetCellFormat(Border:BL));
                count ++;
            }
        ...
        private int WriteHeader(int rowIndex, int colIndex)
        {
            ...
            excel.CellValue(rowIndex, colIndex + 1, "ASSIGNED", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:15 ));
            excel.CellValue(rowIndex, colIndex + 2, "FOLLOWUP-DATE", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:20 ));
            excel.CellValue(rowIndex, colIndex + 3, "NOTES", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:50 ));
