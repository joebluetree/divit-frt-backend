...
{
    public class DataEntryStatSExcelFile
    {
        ...
        public DataEntryStatSExcelFile()
        {
            excel = new TextExcel();
        }
        ...
        private void CreateExcelData()
        {
            ...
            foreach (rep_dataentrystat_dto dr in Dt_List)
            {
                ...
                excel.CellValue(rowIndex, colIndex, dr.mbl_category!, excel.GetCellFormat(Border:BL));
                excel.CellValue(rowIndex, colIndex + 1, dr.mbl_mode!, excel.GetCellFormat(Border:BL));
                excel.CellValue(rowIndex, colIndex + 2, dr.rec_created_by!, excel.GetCellFormat(Border:BL));
                excel.CellValue(rowIndex, colIndex + 3, dr.mbl_count!, excel.GetCellFormat(Border:BL));
                if(Category == "SHIPMENT")
                    excel.CellValue(rowIndex, colIndex + 4, dr.mbl_hbl_count!, excel.GetCellFormat(Border:BL));
            ...
            excel.SetColumnBreak(colIndex + 3);
        ...
        private int WriteHeader(int rowIndex, int colIndex)
        {
            ...
            rowIndex += 1;
            excel.CellValue(rowIndex, colIndex + 0, "CREATED BY" , new CellFormat { Style = "B", FontSize = 10 });
            excel.CellValue(rowIndex, colIndex + 1, CreatedBy, new CellFormat { Style = "B", FontSize = 10 });
            rowIndex += 1;
            excel.CellValue(rowIndex, colIndex + 0, "CATEGORY", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:20 ));
            excel.CellValue(rowIndex, colIndex + 1, "GROUP", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:15 ));
            excel.CellValue(rowIndex, colIndex + 2, "CREATED-BY", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:20 ));
            excel.CellValue(rowIndex, colIndex + 3, "COUNT", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:10 ));
            if(Category == "SHIPMENT")
                excel.CellValue(rowIndex, colIndex + 4, "HBL-TOT", excel.GetCellFormat( Border:"TB", Style:"B", FontSize:10, ColumnWidth:10 ));
