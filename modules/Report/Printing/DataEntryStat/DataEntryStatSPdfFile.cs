...
{
    public class DataEntryStatSPdfFile
    {
        iPdfBase pdf = null!;
        ...
        private ColumnFormat Col_MblCount = new();
        private ColumnFormat Col_HblCount = new();
        ...
        private void Writedocument()
        {
            ...
            this.Col_Category = new ColumnFormat { Left = 30, Width = 90 };
            this.Col_Type = new ColumnFormat { Left = 120, Width = 80 };
            this.Col_CreatedBy = new ColumnFormat { Left = 200, Width = 100 };
            if(Category == "SHIPMENT")
            {
                this.Col_MblCount = new ColumnFormat { Left = 300, Width = 50 };
                this.Col_HblCount = new ColumnFormat { Left = 350, Width = 180 };
            }
            if(Category == "AR/AP" || Category == "AN SENT")
            {
                this.Col_MblCount = new ColumnFormat { Left = 300, Width = 230 };
            }
        ...
        private void CreateReport()
        {
            ...
            foreach (rep_dataentrystat_dto dr in Dt_List)
            {
                ...
                float rowHeight = new[] { Line_Height}.Max();

                pdf.AddText(Row, Col_Category.Left, Col_Category.Width, rowHeight, dr.mbl_category!, format);
                ...
                pdf.AddText(Row, Col_MblCount.Left, Col_MblCount.Width, rowHeight, dr.mbl_count!, format);
                if(Category == "SHIPMENT")
                    pdf.AddText(Row, Col_HblCount.Left, Col_HblCount.Width, rowHeight, dr.mbl_hbl_count!, format);
        ...
        private float WriteHeader(float _Row, float _Col)
        {
            ...
            pdf.AddText(currentY, Col_MblCount.Left, Col_MblCount.Width, Line_Height, "COUNT", new TextFormat { Border = "TB", Style = "B", FontSize = 9, Indent = true });
            if(Category == "SHIPMENT")
                pdf.AddText(currentY, Col_HblCount.Left, Col_HblCount.Width, Line_Height, "HBL-TOT", new TextFormat { Border = "TB", Style = "B", FontSize = 9, Indent = true });
