...
//Remark : this file defines functions getList and getRecords of operations for Sea Volume report/summary


namespace Report.Repositories
{
    public class SeaVolumeRepository : ISeaVolumeRepository
    {
        private readonly AppDbContext context;
        private readonly IAuditLog auditLog;
        public SeaVolumeRepository(AppDbContext _context, IAuditLog _auditLog)
        {
        ...
        public async Task<Dictionary<string, object>> GetListAsync(Dictionary<string, object> data)
        {
            try
            {
                ...
                if (data.ContainsKey("mbl_cntr_type"))
                    mbl_cntr_type = data["mbl_cntr_type"].ToString();
                ...
                if (!Lib.IsBlank(mbl_cntr_type))
                {
                    if (mbl_cntr_type == "ALL")
                    {
                        var Types = new[] { "CONSOLE", "FCL", "LCL", "OTHERS"};
                        query = query.Where(w => Types.Contains(w.mbl_cntr_type));
                    }
                    else
                        query = query.Where(w => w.mbl_cntr_type == mbl_cntr_type);
                }

                var totals = query.Select(s => new
                {
                    mbl_agent_name = "TOTAL",
                    mbl_pcs = s.mbl_pcs ?? 0,
                    mbl_20 = s.mbl_20 ?? 0,
                    mbl_40 = s.mbl_40 ?? 0,
                ...
                var totalRow = new rep_seavolume_dto
                {
                    // mbl_cntr_type = "TOTAL",
                ...
                List<rep_seavolume_dto> Records = new List<rep_seavolume_dto>();
                ...
                if (mbl_report_type == "DETAIL")
                {
                    ...
                    if (!isPrint)
                    {
                        ...
                        .Select(e => new rep_seavolume_dto
                        {
                            mbl_id = e.mbl_id,
                    ...
                    if (mbl_format == "AGENT" && isPrint)
                    {
                        ...
                        foreach (var agentGroup in agentGroups)
                        {
                            ...
                            Records.Add(new rep_seavolume_dto
                            {
                                IsAgentTitle = true,
                ...
                if (mbl_report_type == "SUMMARY")
                {
                    if (mbl_format == "AGENT")
                    {
                        Records = await query
                        .GroupBy(g => new { g.mbl_agent_id, g.agent!.cust_name })
                        .Select(m => new rep_seavolume_dto
                        {
                    ...
                    if (mbl_format == "OPERATION GROUP")
                    {
                        ...
                        var SummaryTotal = new rep_seavolume_dto
                        {
                            mbl_mode = OpSummary.FirstOrDefault()?.mbl_mode,
                            mbl_pcs = OpSummary.Sum(x => x.mbl_pcs),
                            mbl_20 = OpSummary.Sum(x => x.mbl_20 ?? 0),
                            mbl_40 = OpSummary.Sum(x => x.mbl_40 ?? 0),
                            mbl_40hq = OpSummary.Sum(x => x.mbl_40hq ?? 0),
                            mbl_45 = OpSummary.Sum(x => x.mbl_45 ?? 0),
                            mbl_teu = OpSummary.Sum(x => x.mbl_teu ?? 0),
                            mbl_cbm = OpSummary.Sum(x => x.mbl_cbm ?? 0),
        ...
        public filesm ProcessPdfFileAsync(List<rep_seavolume_dto> Records, string title, int company_id, string user_name, int branch_id, Dictionary<string, string> searchInfo)
        {
            var Dt_List = Records;
            if (Dt_List.Count <= 0)
                throw new Exception("Print List Records error");

            SeaVolumePdfFile bc = new SeaVolumePdfFile
            {
                Dt_List = Dt_List,
                ...
                ShipType = searchInfo.ContainsKey("mbl_cntr_type") ? searchInfo["mbl_cntr_type"] : "",
            };
            bc.Process();
        ...
        public filesm ProcessExcelFileAsync(List<rep_seavolume_dto> Records, string title, int company_id, string user_name, int branch_id, Dictionary<string, string> searchInfo)
        {
            var Dt_List = Records;
            if (Dt_List.Count <= 0)
                throw new Exception("Excel List Records error");

            SeaVolumeExcelFile bc = new SeaVolumeExcelFile
            {
                Dt_List = Dt_List,
                report_folder = Path.Combine(Lib.rootFolder, Lib.TempFolder, CommonLib.GetSubFolderFromDate()),
                Title = title,
