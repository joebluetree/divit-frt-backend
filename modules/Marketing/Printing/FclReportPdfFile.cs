...
namespace Marketing.Printing
{
    public class FclReportPdfFile
    {
        ...
        private ColumnFormat Col_Cutoff = new();
        private ColumnFormat Col_CntrType = new();
        private ColumnFormat Col_OF = new();
        private ColumnFormat Col_PSS = new();
        private ColumnFormat Col_BAF = new();
        private ColumnFormat Col_ISPS = new();
        private ColumnFormat Col_Haulage = new();
        private ColumnFormat Col_IFS = new();
        private ColumnFormat Col_Total = new();

        public FclReportPdfFile()
        {
        ...
        private void Writedocument()
        {
            ...
            this.Col_Cutoff = new ColumnFormat { Left = 350, Width = 90 };
            this.Col_CntrType = new ColumnFormat { Left = 440, Width = 40 };//20
            this.Col_OF = new ColumnFormat { Left = 480, Width = 40 };
            this.Col_PSS = new ColumnFormat { Left = 520, Width = 50 };
            this.Col_BAF = new ColumnFormat { Left = 570, Width = 50 };
            this.Col_ISPS = new ColumnFormat { Left = 620, Width = 50 };
            this.Col_Haulage = new ColumnFormat { Left = 670, Width = 60 };
            this.Col_IFS = new ColumnFormat { Left = 730, Width = 50 };
            this.Col_Total = new ColumnFormat { Left = 780, Width = 50 };
            /// need modification
        ...
        private void CreateReport()
        {
            ...
            foreach (mark_qtnd_fcl_dto dr in Dt_List)
            {
                ...
                float TransitTimeHeight = pdf.MeasureWrappedTextHeight(Row, Col_Cutoff.Left, Col_Cutoff.Width, Line_Height, dr.qtnd_trans_time!, format);
                float CutoffHeight = pdf.MeasureWrappedTextHeight(Row, Col_Cutoff.Left, Col_Cutoff.Width, Line_Height, dr.qtnd_cutoff!, format) + TransitTimeHeight;
                float CntrTypeHeight = pdf.MeasureWrappedTextHeight(Row, Col_CntrType.Left, Col_CntrType.Width, Line_Height, dr.qtnd_pod_name!, format);
                float OFHeight = pdf.MeasureWrappedTextHeight(Row, Col_OF.Left, Col_OF.Width, Line_Height, dr.qtnd_of!, format);

                float rowHeight = Line_Height * 2;// min row hight
                rowHeight = new[] { PolNameHeight, PodNameHeight, RoutingHeight, CarrierHeight, ETDHeight, CutoffHeight, TransitTimeHeight, rowHeight }.Max();
                ...
                pdf.AddText(Row, Col_Cutoff.Left, Col_Cutoff.Width, rowHeight, dr.qtnd_trans_time!, new TextFormat { Border = "B", FontSize = 9, Indent = true });
                pdf.AddText(Row + TransitTimeHeight, Col_Cutoff.Left, Col_Cutoff.Width, rowHeight, dr.qtnd_cutoff!, new TextFormat { FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_CntrType.Left, Col_CntrType.Width, rowHeight, dr.qtnd_cntr_type!, new TextFormat { Border = "B" , FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_OF.Left, Col_OF.Width, rowHeight, dr.qtnd_of!, new TextFormat { Border = "B" , Style = "R", FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_PSS.Left, Col_PSS.Width, rowHeight, dr.qtnd_pss!, new TextFormat { Border = "B", Style = "R", FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_BAF.Left, Col_BAF.Width, rowHeight, dr.qtnd_baf!, new TextFormat { Border = "B", Style = "R", FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_ISPS.Left, Col_ISPS.Width, rowHeight, dr.qtnd_isps!, new TextFormat { Border = "B", Style = "R", FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_Haulage.Left, Col_Haulage.Width, rowHeight, dr.qtnd_haulage!, new TextFormat { Border = "B" , Style = "R", FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_IFS.Left, Col_IFS.Width, rowHeight, dr.qtnd_ifs!, new TextFormat { Border = "B" , Style = "R", FontSize = 9, Indent = true });
                pdf.AddText(Row, Col_Total.Left, Col_Total.Width, rowHeight, dr.qtnd_tot_amt!, new TextFormat { Border = "B", Style = "R", FontSize = 9, Indent = true });//Style = "R",
                if(!Lib.IsBlank(BL))
                    pdf.AddText(Row, Col_PolName.Left, Row_Width, rowHeight, "", new TextFormat { Border = BL, Style = "R", FontSize = 9, Indent = true });//Style = "R",
        ...
        private float WriteHeader(float _Row, float _Col)
        {
            ...
            if (!IsAttachment)
            {
                ...
                pdf.AddText(currentY, Col_Cutoff.Left, Col_Cutoff.Width, RowHeight, "T/T", new TextFormat { Border = "T", Style = "B", FontSize = 10, Indent = true });
                pdf.AddText(currentY + Line_Height, Col_Cutoff.Left, Col_Cutoff.Width, RowHeight, "CUT-OFF", new TextFormat { Border = "", Style = "B", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_CntrType.Left, Col_CntrType.Width, RowHeight, "CNTR TYPE", new TextFormat { Border = "T", Style = "B", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_OF.Left, Col_OF.Width, RowHeight, "OF", new TextFormat { Border = "T", Style = "RB", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_PSS.Left, Col_PSS.Width, RowHeight, "PSS", new TextFormat { Border = "T", Style = "RB", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_BAF.Left, Col_BAF.Width, RowHeight, "BAF", new TextFormat { Border = "T", Style = "RB", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_ISPS.Left, Col_ISPS.Width, RowHeight, "ISPS", new TextFormat { Border = "T", Style = "RB", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_Haulage.Left, Col_Haulage.Width, RowHeight, "HAULAGE", new TextFormat { Border = "T", Style = "RB", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_IFS.Left, Col_IFS.Width, RowHeight, "IFS", new TextFormat { Border = "T", Style = "RB", FontSize = 10, Indent = true });
                pdf.AddText(currentY, Col_Total.Left, Col_Total.Width, RowHeight, "TOTAL", new TextFormat { Border = "T", Style = "RB", FontSize = 10, Indent = true });
        ...
        private float FooterPadding(float currentY, float lastRowHeight)
        {
            ...
            float maxFooter = 500;
            bool printAtta = false;
            float remainingHeight = maxFooter - currentY;
            ...
            foreach (var remk in RemkList)
            {
                ...
                printAtta = IsPageBreak(currentY, maxFooter, remkCount, Line_Height, 12);
