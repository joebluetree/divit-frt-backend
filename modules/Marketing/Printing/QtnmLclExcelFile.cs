    ...
    public class ProcessExcelFile
    {
        ...
        private void CreateExcelData()
        {
            ...
            foreach (mark_qtnm_dto dr in Dt_List)
            {
                ...
                excel.CellValue(rowIndex, colIndex + 4, dr.qtnm_pol_name!, new CellFormat { Border = "A", FontSize = 9, ColumnWidth = 15 });
                excel.CellValue(rowIndex, colIndex + 5, dr.qtnm_pod_name!, new CellFormat { Border = "A", FontSize = 9, ColumnWidth = 15 });
                excel.CellValue(rowIndex, colIndex + 6, dr.qtnm_pld_name!, new CellFormat { Border = "A", FontSize = 9, ColumnWidth = 30 });
                excel.CellValue(rowIndex, colIndex + 7, dr.qtnm_move_type!, new CellFormat { Border = "A", FontSize = 9, ColumnWidth = 12 });
                excel.CellValue(rowIndex++, colIndex + 8, dr.qtnm_commodity!, new CellFormat { Border = "A", FontSize = 9, ColumnWidth = 30 });
            }
        ...
        private int WriteHeader()
        {
            int rowIndex = 0;
            int colIndex = 0;
            int col_count = 2; // Column count to merge
            ...
            rowIndex += 1;
            excel.CellValue(rowIndex, colIndex, "QUOTE TO:" + QuoteTo, new CellFormat { FontSize = 10, ColumnWidth = 50 });
            excel.CellValue(rowIndex, colIndex + 1, "QUOTE NO:" + QuoteNo, new CellFormat { FontSize = 10, ColumnWidth = 50 });
            rowIndex += 1;
            excel.CellValue(rowIndex, colIndex, "PLACE OF DELIVERY:" + Quotepld, new CellFormat { FontSize = 10, ColumnWidth = 50, Merge = 1 });
            rowIndex += 1;
            ...
            excel.CellValue(rowIndex, colIndex + 4, "POL", new CellFormat { Border = "A", Style = "B", FontSize = 10, ColumnWidth = 15 });
            excel.CellValue(rowIndex, colIndex + 5, "POD", new CellFormat { Border = "A", Style = "B", FontSize = 10, ColumnWidth = 15 });
            excel.CellValue(rowIndex, colIndex + 6, "PLACE OF DELIVERY", new CellFormat { Border = "A", Style = "B", FontSize = 10, ColumnWidth = 30 });
            excel.CellValue(rowIndex, colIndex + 7, "MOVE", new CellFormat { Border = "A", Style = "B", FontSize = 10, ColumnWidth = 12 });
            excel.CellValue(rowIndex, colIndex + 8, "COMMODITY", new CellFormat { Border = "A", Style = "B", FontSize = 10, ColumnWidth = 30 });
            rowIndex += 1;
            return rowIndex;
        }
